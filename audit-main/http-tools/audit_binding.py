...
COMMIT = '126b947dcf8fedd553fabd2b4de384bc6d483739'
TREE = '01e6e1b5e614f3a29a86aa816595da46c25dab16'
SOURCE = '9374f7fab315a2b9145d699cb4548fb7807b1380'
SOURCE_TREE = 'd4338abbf5c65809af38a398cedf0d9fcae86b7f'
INVENTORY_SHA = '172a4c080e6d19ffc68243225c0fcc951ef7555d7b55de2a63cb8693c80c1200'
EXPECTED_FILES = 4005
EXPECTED_BYTES = 615708592
PRODUCTION_RUN = 35811757995
...
def validate_execution_binding():
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        ...
        require(receipt['format'] == 'revealline-metadata-pages-artifact.v1' and receipt['controllerCommit'] == COMMIT and receipt['controllerTree'] == TREE and receipt['currentVersion'] == 'v0.89.0' and receipt['gameSourceRevision'] == SOURCE and receipt['qualifiedSourceTree'] == SOURCE_TREE, 'Wrong frozen receipt source/controller')
        require(receipt['publishable'] is True and receipt['browserAdmissionsRequired'] is True and receipt['admittedArchives'] == 57 and receipt['historicalBridges'] == 132, 'Receipt is not actual production eligible')
        ...
        for name, field in [('publication.json', 'configurationSha256'), ('catalog.json', 'catalogSha256')]:
            ...
            if name == 'catalog.json':
                ...
                require(len(releases) == 133 and len({row['version'] for row in releases}) == 133 and sum(row['version'] == 'v0.89.0' for row in releases) == 1, 'Wrong complete catalog coverage')
        extraction = json.loads(archive.read('zip-receipt.json'))
        require(extraction['version'] == 'v0.89.0' and extraction['gameSourceRevision'] == SOURCE and extraction['distributionSha256'] == 'afabe1075b6c2bbdc9c1aae98ffbfa462a8a3f1b2478ae36aeac65d60afac622' and extraction['manifestSha256'] == '397d8fd8f1ab32de6c4d1cfe905a3e3a161691ff8b488ac74c84d9171cae572a' and extraction['crcAndHashesVerified'] is True, 'Frozen ZIP source/hash identity mismatch')
