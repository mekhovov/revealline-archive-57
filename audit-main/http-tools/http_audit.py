"""Read every hash-pinned main v0.89.0 HTTP body without persisting payloads.
...
from audit_binding import validate_execution_binding, candidate_inventory, INVENTORY_SHA, EXPECTED_FILES, EXPECTED_BYTES
...
def fetch_once(row, attempt, opener, *, deadline, clock=time.monotonic):
    ...
    try:
        ...
        request = urllib.request.Request(url, headers={
            ...
            'User-Agent': 'RevealLine-main-v0890-byte-audit/2.0',
        })
...
def main():
    ...
    parser.add_argument('--run', action='store_true', help='Audit the fixed complete production main inventory')
    ...
    if args.prepare:
        ...
        for row in rows:
            validate_row(row)
        print(json.dumps({'status': 'PREPARED_ACTUAL_BINDING_REQUIRED', 'files': len(rows), 'bytes': EXPECTED_BYTES, 'networkRequests': 0}))
        return 0
    ...
    report = {
        'status': 'FAIL' if failed or not unchanged_authority or audit_deadline_exceeded else 'PASS', 'base': BASE, 'controllerCommit': COMMIT,
        ...
        'controllerTree': admitted_binding['request']['controllerTree'],
        ...
        'scope': 'Every actual main production inventory HTTP body and explicit MIME, including historical bridges and hidden metadata. Independent of browser/offline or physical-device acceptance; other archive canonical inventories are not reread by this main-origin audit.',
    }
