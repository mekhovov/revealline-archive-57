"""Read-only, fail-closed intake of the actual Archive57 initial-deployment receipts."""
import hashlib
import io
import json
import os
...
BASE = 'https://mekhovov.github.io/revealline-archive-57/'
REPO = 'mekhovov/revealline-archive-57'
TREE = '5d00558b78f917cd7bb40d718fbcf0ef55b2ed86'
INVENTORY_SHA = '57a7eb6dd8667380cb4785a9b305bc58715623c4793afb96cb9f318d656bd42b'
LOCK_SHA = '8a88eb49ed3ba9dab821f128207e558dcb6e5caf80e47a32b14fbcc5fe8b3ab3'
...
def candidate_inventory():
    ...
    require(len(rows) == len(current) == 1088 and sum(r['bytes'] for r in rows) == 590907371, 'Wrong finite inventory')
...
def validate_execution_binding():
    ...
    require(request['format'] == 'revealline-archive57-http-request.v1' and request['reviewed'] is True, 'Actual request requires explicit review')
    ...
    require(artifact['name'] == 'archive57-verification-receipts' and artifact['expired'] is False and artifact['workflow_run']['id'] == run_id and artifact['workflow_run']['head_sha'] == commit, 'Wrong receipt artifact run')
    ...
    with zipfile.ZipFile(io.BytesIO(zip_raw)) as archive:
        names = ['receipt.json', 'expected-inventory.json', 'zip-receipt-v0.88.0.json']
